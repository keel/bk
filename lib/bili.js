...
async function main() {
  const newCookie = readlineSync.question('请输入BiliBili新cookie(包含"_uuid="):');
  ...
  const outFormatArr = ['none', null, 'mp3'];
  const outFormatIndex = parseInt(readlineSync.keyInSelect(['mp4', 'mp3'], '选择转码&合并的输出格式？')) + 1;
  ...
  try {
    if (pNum !== -999) {
      ...
      await downloadOne(aid, vOne.cid, vOne.part, pNum + 1, quality, outFormat);
    } else {

...
const checkCookieFile = async function() {
  ...
  if (!Fs.existsSync(cookieFilePath)) {
    ...
    isSetCookie = parseInt(await readline.keySelectSync(['使用空cookie', '我已录入新cookie']));
  } else {
    isSetCookie = 1;
  }
  if (isSetCookie < 0) {
...
async function main() {
  ...
  let pNum = 0;
  let aid = await readline.questionSync('请输入BiliBili的av/BV号(或url):');
  if (aid.length < 6) {
  ...
  if (videoInfo.pages && videoInfo.pages.length > 1) {
    ...
    pageArr.push('===>下载所有分P');
    const pNumStr = await readline.keySelectSync(pageArr, '请选择分P号:');
  ...
  const qualityIndex = await readline.keySelectSync(qualityDesArr, '请选择下载质量？');
  if (qualityIndex < 0) {
    console.log('退出');
    return;
  }
  ...
  const outFormatIndex = await readline.keySelectSync(outFormatArr, '请选择转码&合并的输出格式？');
  if (outFormatIndex < 0) {
    console.log('退出');
    return;
  }

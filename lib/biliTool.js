...
// const apiUrlB = 'https://api.bilibili.com/x/player/wbi/playurl?';


const getVideoInfo = async function(bvid) {
  // console.log('myCookie:',myCookie);
  const infoStr = '' + await ktool.promi(ktool.httpGet)(apiUrlA + 'bvid=' + bvid, {
    'headers': {
      'User-Agent': UA,
      'Cookie': myCookie
    }
  ...
  // console.log('getVideoInfo:%j',info);
  if (info.data) {
    return info.data;
  }
...
const getDownloadInfo = async function(bvid, cid, qualityNum) {
  // console.log('myCookie:',myCookie);
  const infoStr = '' + await ktool.promi(ktool.httpGet)(apiUrlB + `bvid=${bvid}&cid=${cid}&qn=${qualityNum}&otype=json`, {
    'headers': {
      'User-Agent': UA,
      'Cookie': myCookie
    }
...
const httpGet = function(uri, bvid, filename, str, writeStream, reject) {
  // console.log('uri:---->', uri);
  ...
  httpWraper.get(opts2, function(res2) {
    if (res2.statusCode === 302) {
      ...
      res2.destroy();
      httpGet(u2, bvid, filename, str, writeStream, reject);
      return;
    }
...
function downloadFile(bvid, totalSize, totalTime, uri, filename) {
  return new Promise(function(resolve, reject) {
    // console.log('downloadFile: uri:'+uri+',filename:'+filename,bvid,totalSize,totalTime);
    ...
    httpGet(uri, bvid, filename, str, writeStream, reject);
